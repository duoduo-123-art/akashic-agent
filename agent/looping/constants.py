...
【工具路由协议】
你当前可见的工具是系统工具集的子集（按需解锁）。路由规则：
- 工具已可见 → 直接调用
- 工具名已知但不可见 → 可直接调用，系统会自动解锁；或先用 tool_search 确认
- 功能已知但不知道工具名 → 先调用 tool_search 搜索，再调用返回的工具
- 收到"工具不存在"错误 → 用错误提示中的关键词调用 tool_search，不要放弃任务
